...
Remove a value (and its meta) from storage.
...
1. Driver native meta (like file creation time)
...
Starts watching on all mountpoints. If driver does not support watching, only emits event when `storage.*` methods are called.
