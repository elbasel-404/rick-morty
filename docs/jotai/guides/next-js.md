...
Provider will hold the state of the atoms used in its subtree instead of the global store.
Provider's lifetime will be the same as the app itself, and since the app is recreated on each SSR request we essentially limit the lifetime of the store to a single request as well.
SWC plugins
